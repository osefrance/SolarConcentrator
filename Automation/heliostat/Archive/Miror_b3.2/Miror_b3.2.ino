  ...
  void loop() {
   ...
   if (pushbutstate == HIGH ) {// triggers to move by a Tma angle target in full steps, stops 2 degrees before target, then scan
       ...
       retard = 10;//small delay when moving toward a target
   ...
   retard = 50;//small delay between steps impulses
  ...
  Serial.println(",");     
   ...
   void search(){//               MODULE LOOKS FOR MIROR POSITION FOR REFLECTION ON RECEPTOR
     ...
     digitalWrite(dirPin,HIGH);   // Set Dir high
    retard = 1000;// delay with rest time for no vibration photoresistance reading
    while (peaks < 2){//routine while two candidate peaks havbe not yet been identified
      ...
      if (PhLA > PhLmax) {//search for light peaks of proper intensity and distances
      ...
      peaks = peakL + peakR;//=2 if two peaks identified
      turn();
      ...
      if(peaks == 2){
        ...
        if(x<40 || x>80){ // range of acceptable distance between peaks right and left(65 calculated)
          PhRmax = 0;// peaks are likely to be noise, scanning continue
          PhLmax = 0;// peaks are likely to be noise, scanning continue
          peaks = 0;
        }
      else{
        y = micstepscount - (aRmax + aLmax)/2;//number of steps corresponding to reflection on the center of the receptor
        Serial.print("steps return");
        Serial.print(",");
        Serial.print(y);
        Serial.print(",");
        digitalWrite(dirPin,LOW); // change direction
        for (z = 0; z < y; z++) {//goes back the number of steps for reflection centered on the receptor
          turn();
